...
{	
	...
	public FormMain(Competition competition, JDesktopPane desktop)
	{
		...
		bUpJoueur.setToolTipText("Classe le joueur d'un niveau supérieur dans la liste.");
		bDownJoueur.setToolTipText("Classe le joueur d'un niveau inférieur dans la liste.");
		bAddJoueur.setToolTipText("Ajoute un joueur en dernière place du tableau sélectionné.");        
        cbTournois.setToolTipText("Liste des formules.");
        bCreer.setToolTipText("Création d'un nouveau tableau.");
        bLancer.setToolTipText("Démarre le tableau sélectionné.");
        bReload.setToolTipText("Recharge la liste des formules afin de prendre en compte les nouvelles formules.");
		bSupprimer.setToolTipText("Supprime le tableau sélectionné.");
	...
	int ligne = 0;
		...
		constraints.ipady = 0;
		constraints.insets = new Insets(2, 2, 2, 2);
		panel.add(new JLabel("Liste des tableau crées"), constraints);
			...
			if(tabName!=null && !tabName.equals(""))
			{
                ...
                if(tableau==null)
                {
                    JOptionPane.showMessageDialog(
                            getContentPane(),
                            "Ce nom de tableau existe déjà ! Veuillez choisirun autre nom !",
                            "Erreur !",
	...
	public void addJoueur(ActionEvent e)
	{
		...
		if(tableau==null)
		{
			JOptionPane.showMessageDialog(
					getContentPane(),
					"Vous devez sélectionner le tableau dans lequel les joueurs seront ajoutés",
					"Attention !",
		...
		if(list==null||list.size()<1)
		{
			JOptionPane.showMessageDialog(
					getContentPane(),
					"Vous devez sélectionner des joueurs à ajouter dans le tableau.",
					"Attention !",
					JOptionPane.INFORMATION_MESSAGE);
			return;
		}
